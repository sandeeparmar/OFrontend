    ...
    const renderDataVisualization = () => {
        if (loading) {
            return (
                <div className="flex justify-center items-center h-40 xs:h-48 sm:h-56 md:h-64 lg:h-72 xl:h-80">
                    <div className="animate-spin rounded-full h-8 w-8 xs:h-10 xs:w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 border-t-2 border-b-2 border-blue-400"></div>
                    <span className="ml-3 text-sm xs:text-base sm:text-lg text-gray-300">Loading data...</span>
                </div>
            );
        }
        ...
        return (
            <div className="bg-gray-800 p-4 rounded-lg shadow text-center">
                <p className="text-gray-400 text-sm xs:text-base">No data available for this float</p>
            </div>
        );
    };

    return (
        <div className="pb-4 flex flex-col bg-gradient-to-br from-gray-900 to-gray-800 mt-20" >
            <div className="flex m-3 p-3 gap-3 justify-center px-5 bg-gray-800 border border-gray-700 rounded-xl">
                <button
                    ...
                    className={`flex items-center space-x-2 px-5 py-3 rounded-full transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 ${
                        viewMode === 'map'
                            ? 'bg-gradient-to-r from-cyan-600 to-violet-700 text-white shadow-lg shadow-violet-700/40'
            ...
            {viewMode === 'map' && (
                <div className="p-2 xs:p-3 sm:p-4 md:p-5 lg:p-6 mb-3 xs:mb-4 sm:mb-5 md:mb-6">
                    <div className="bg-gray-800 rounded-lg shadow-xl overflow-hidden border border-gray-700">
                        <h2 className="text-lg font-semibold mb-3 p-4 bg-gray-700 text-gray-200 border-b border-gray-600">
                ...
                <div className="space-y-4 p-5">
                    <div className="bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-700">
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-400">
                            <p className="flex items-center gap-2"><MapPin className="w-4 h-4 text-blue-400" /> {selectedLocation.latitude}°N, {selectedLocation.longitude}°E</p>
                            <p className="flex items-center gap-2"><Waves className="w-4 h-4 text-blue-400" /> {selectedLocation.data_mode}</p>
                            <p className="flex items-center gap-2"><BarChart3 className="w-4 h-4 text-blue-400" /> {selectedLocation.measurements} measurements</p>
                        </div>
                        <div className="mt-6 flex flex-wrap gap-4 justify-center">
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-3 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 ${
                                    plotMode === 'temperature' ? 'bg-orange-600 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-orange-700/30'
                                }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-3 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 ${
                                    plotMode === 'salinity' ? 'bg-teal-600 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-teal-700/30'
                                }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-3 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 ${
                                    plotMode === 'all' ? 'bg-blue-700 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-blue-700/30'
                                }`}
                            >
                                <BarChart3 className="w-5 h-5" />
                                <span>All Plots</span>
            ...
            {!selectedLocation && viewMode === 'data' && (
                <div className="bg-gray-800 rounded-lg shadow-xl p-6 text-center mx-5 my-auto flex flex-col items-center justify-center min-h-[250px] border border-gray-700">
